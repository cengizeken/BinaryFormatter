	...
	public class Schema {
		...
		object ParsePrimitive(BinaryReader reader, NodeType type, out int size) {
			...
			switch (type) {
				...
				case NodeType.UShort:
					size = 2;
					return this.isLittleEndian ? reader.ReadUInt16() : reader.ReadUInt16().ReverseBytes();
				case NodeType.Short:
					size = 2;
					return this.isLittleEndian ? reader.ReadInt16() : reader.ReadInt16().ReverseBytes();
				case NodeType.UInt:
					size = 4;
					return this.isLittleEndian ? reader.ReadUInt32() : reader.ReadUInt32().ReverseBytes();
				case NodeType.Int:
					size = 4;
				...
				case NodeType.ULong:
					size = 8;
					return this.isLittleEndian ? reader.ReadUInt64() : reader.ReadUInt64().ReverseBytes();
				case NodeType.Long:
					size = 8;
					return this.isLittleEndian ? reader.ReadInt64() : reader.ReadInt64().ReverseBytes();
				case NodeType.Float:
					size = 4;
			...
			while ((token = lexer.NextToken()) != null) {
				...
				switch (token.Value) {
					...
					case "schema":
						...
						break;
					case "order":
						endiannesToken = lexer.NextToken();
						if(endiannesToken.Value == "littleendian")
                        {
							this.isLittleEndian = true;
                        }
						if (endiannesToken.Value == "bigendian")
						{
							this.isLittleEndian = false;
						}
						token = lexer.NextToken();///read comma
						//ReadSchemaBlock(node);
						break;
					default:
		...
		void ReadBody(Node node) {
			...
			if (token.Value == "littleendian")
			{
				...
				throw new Exception($"({token.Line}:{token.Column}) Expected [{{].");
			/*if (token.Type == TokenType.Endianness)
			{
				if (token.Value == "be")
				{
				...
				if (control.Contains(token.Value)) {
					...
					switch (token.Value) {
						...
						case "until":
							...
							break;
						/*case "be":
							child.Type = NodeType.Endianness;
							break;*/
						/*case "for":
